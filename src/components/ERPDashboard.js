// ERP Dashboard - Role-Based Access Control - Version 4.0
...
export default function ERPDashboard({ onPOSAccess, onSalesAccess, onHRAccess, onLegalAccess, onColdChainAccess, onInventoryAccess, onLogout }) {
  ...
  const { canAccessModule, getRoleDisplayName } = useRole()
  ...
  // Define all available modules with their access keys
  const allModules = [
    {
      key: "hr",
      icon: (
    ...
    },
    {
      ...
      onAccess: null // Not implemented yet
    },
    {
      key: "ecommerce",
      icon: (
      ...
      title: "E-commerce",
      description: "Online store administration panel",
      color: "#8B5CF6", // Purple for E-commerce
      onAccess: null // Not implemented yet
    },
    {
      key: "coldchain",
    ...
    },
    {
      ...
      onAccess: () => window.open('/clear-data', '_blank')
    }
  ]

  // Filter modules based on user permissions
  const availableModules = allModules.filter(module => canAccessModule(module.key))

  const handleModuleClick = (module) => {
    console.log(`[Role-Based v4.0] Module clicked: ${module.title} by ${getRoleDisplayName()}`)
    
    if (module.onAccess) {
      module.onAccess()
    } else {
      console.log(`Module ${module.title} not yet implemented`)
    }
  }
  ...
  return (
    <div className="erp-dashboard">
      {/* Header Section */}
      <div className={`header-section ${headerVisible ? 'animate' : ''}`}>
        <div className="header-content">
          {/* User Role Badge */}
          <div className="role-badge">
            <span className="role-icon">👤</span>
            <span className="role-text">{getRoleDisplayName()}</span>
            <span className="role-type">({userRole?.role || 'USER'})</span>
          </div>
          
          {/* Main Title */}
          <h1 className="main-title">CoreERP Dashboard</h1>
          <p className="main-subtitle">Smart ERP System for Small & Medium Businesses</p>
